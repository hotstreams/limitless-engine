...
std::string MaterialShaderDefineReplacer::getPropertyDefines(const Material& material) {
    ...
    if (material.getDefaultComputation()) {
...
std::string MaterialShaderDefineReplacer::getModelDefines(InstanceType model_shader) {
    ...
    defines.append("#define " + MODEL_DEFINE[model_shader] + '\n');

    for (auto& [index, name] : getNameMappingFromInstanceType(model_shader)) {
        std::transform(name.begin(), name.end(), name.begin(), ::toupper);
        defines.append("#define ENGINE_VERTEX_" + name + '\n');
    }
