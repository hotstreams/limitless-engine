...
void process(
    ...
    c_angle = DECODE_ROTATION(control);
#endif

    float blend = 0.0;
#ifdef ENGINE_MATERIAL_TERRAIN_BLENDING
    blend = DECODE_BLEND(control);
#endif
    ...
    vec2 c_cs_angle = vec2(1.0, 0.0);
#ifdef ENGINE_MATERIAL_TERRAIN_DETILING
    c_cs_angle = vec2(cos(c_angle), sin(c_angle));
    ...
    if (blend < 1.0) {
        ...
        id_cs_angle = vec2(cos(id_detile.x), sin(id_detile.x));

        id_uv = rotate_vec2(fma(i_uv, vec2(id_scale), -uv_center), id_cs_angle) + uv_center + id_detile.y - 0.5;
        ...
        id_weight = exp2(blend_sharpness * log2(weight + id_w + alb.a)) * weight;
#else
        id_weight = (weight + id_w) * weight;
#endif
    ...
    if (blend > 0.0 && texture_id[1] != texture_id[0]) {
        ...
        vec2 id_uv_detiled = rotate_vec2(fma(i_uv, vec2(id_scale), -uv_center), id_cs_angle) + uv_center + id_detile.y - 0.5;
        id_uv = id_uv_detiled;
        ...
        float id_weight = 0.0;
#ifdef ENGINE_MATERIAL_TERRAIN_HIGH_BLENDING
        id_weight = exp2(blend_sharpness * log2(weight + id_w + alb.a * clamp(world_normal, 0.0, 1.0))) * weight;
#else
        id_weight = (weight + id_w) * weight;
#endif
        mat.albedo_height = fma(alb, vec4(id_weight), mat.albedo_height);
...
void calculateTerrain(inout MaterialContext mctx) {
    ...
    index[0] = ivec2(mod(terrain_texel_base + offsets.xy, vec2(terrain_size)));
    index[1] = ivec2(mod(terrain_texel_base + offsets.yy, vec2(terrain_size)));
    index[2] = ivec2(mod(terrain_texel_base + offsets.yx, vec2(terrain_size)));
    index[3] = ivec2(mod(terrain_texel_base + offsets.xx, vec2(terrain_size)));
    ...
    bool tile_bilerp_candidate = region_mip < tile_bilerp_multiplier;
    ...
    if (normal_bilerp) {
        ...
        float h4 = fetchTerrainHeight(ivec2(mod(terrain_texel_base + offsets.yz, vec2(terrain_size))));
        float h5 = fetchTerrainHeight(ivec2(mod(terrain_texel_base + offsets.zy, vec2(terrain_size))));
        float h6 = fetchTerrainHeight(ivec2(mod(terrain_texel_base + offsets.zx, vec2(terrain_size))));
        float h7 = fetchTerrainHeight(ivec2(mod(terrain_texel_base + offsets.xz, vec2(terrain_size))));
    ...
    bool tile_bilerp_enabled = tile_bilerp_candidate && bool(enable_tile_bilerp);
#else
    bool tile_bilerp_enabled = false;
#endif
    ...
    float base_weight = tile_bilerp_enabled ? weights[3] : 1.0;
